...
object SparkUtils {
  ...
  def getPageViews(clickStreamRdd:RDD[ClickStream]) = {

    clickStreamRdd.map(record => record.getRequestUri.toString)
      .filter(_.length != 0)
      .map(requestURI => {
        logger.debug("requestURI: " + requestURI)
        val productURI = requestURI.split(" ")(1)
  ...
  def getStatusCount(clickStreamRdd:RDD[ClickStream]) = {
    clickStreamRdd.map(record => {
      val status = record.getStatus.toString
      logger.debug("status_code: " + status)
      (status, 1)
    }).reduceByKey(_ + _)
  }
  ...
  def getReferrer(clickStreamRdd:RDD[ClickStream]) = {
    clickStreamRdd.map(record => {
      val referrer = record.getReferrer.toString
      logger.debug("referrer: " + referrer)
      (referrer, 1)
    }).reduceByKey(_ + _)
  }
  ...
  def getVisitsByCounrty(clickStreamRdd:RDD[ClickStream], countryCodeBroadcast: Broadcast[mutable.Map[String, String]]) = {

    clickStreamRdd.map(record => record.getIpv4.toString)
      .filter(_.length != 0)
      .map(ipaddress => {
        logger.debug("ipaddress: " + ipaddress)
        val countryCodeMap =  countryCodeBroadcast.value
